...
router.get("/fetchCalendar", ensureAuthenticated, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(
        "An error has occured while handling the request. Please try again later!"
      );
...
router.post("/fetchProjectData", ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (
      fetchedProject &&
      fetchedProject.members.some((member) => member.userId == req.user._id)
    ) {
    ...
    logger.error(
      `Failed to fetch project ${req.body.projectId} data for user ${req.user.id}:\n${error}`
    );
  }
});
...
router.post("/fetchProjectTaskData", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const projectMember = fetchedProject.members.some(
    ...
    if (fetchedProject && taskData && projectMember) {
      ...
      if (
        projectCreator == req.user._id ||
        taskData.assingedTo == req.user.username
      )
        canEdit = true;

      res.json({ taskData, canEdit });
      logger.debug(
        `Fetched project task ${taskId} data of project ${projectId} for user ${req.user.id}`
      );
      console.log(
        `Fetched project task ${taskId} data of project ${projectId} for user ${req.user.id}`
      );
...
router.post("/updateProjectTaskData", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const projectMember = fetchedProject.members.find(
      (member) => member.userId == req.user._id
    );
    const projectTaskIndex = fetchedProject.tasks.findIndex(
    ...
    if (
      ...
      projectMember &&
      projectTaskIndex != -1 &&
      (fetchedProject.tasks[projectTaskIndex].assingedTo == req.user.username ||
        projectMember.isCreator)
    ) {
...
router.post("/deleteProject", ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (fetchedProject && creatorId == req.user._id) {
      ...
      logger.debug(
        `Deleted project ${projectId} as requested by user ${req.user.id}`
      );
      console.log(
        `Deleted project ${projectId} as requested by user ${req.user.id}`
      );
...
router.post("/addTask", ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.error(
      `Failed to add new task "${req.body.taskName}" for user ${req.user.id}:\n${error}`
    );
  }
});
...
router.post("/addProjectTask", ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.error(
      `Failed to add new task "${req.body.taskName}" to project ${req.body.projectId}:\n${error}`
    );
  }
});
...
router.post("/editTask", ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.debug(
      `Edited task ${taskId} for user ${req.user.id}:\nName: ${taskName}\nComment: ${taskComment}`
    );
  } catch (error) {
    console.log(error);
    res
    ...
    logger.error(
      `Failed editing task ${req.body.taskId} for user ${req.user.id}:\nName: ${req.body.taskName}\nComment: ${req.body.taskComment}`
    );
  }
});
...
router.post("/updateTaskStatus", ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.debug(
      `Updated task ${taskId} for user ${req.user.id}:\nCompleted: ${completed}`
    );
  } catch (error) {
    console.log(error);
    res
    ...
    logger.error(
      `Failed updating task ${req.body.taskId} for user ${req.user.id}:\nCompleted: ${req.body.completed}`
    );
  }
});
...
router.post("/deleteTask", ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.error(
      `Failed deleting task ${req.body.taskId} for user ${req.user.id}`
    );
  }
});
...
router.post("/changeDueDate", ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (taskId) {
      ...
      logger.debug(
        `Changed due date of task ${taskId} for user ${req.user.id}`
      );
    ...
    logger.error(
      `Failed changing due date of task ${req.body.taskId} for user ${req.user.id}`
    );
  }
});
...
router.post("/createNewProject", ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (title) {
      let newProjectList = await createNewProject(
        req.user.id,
        req.user.username,
        title
      );
    ...
    logger.error(
      `Failed creating new project ${req.body.title} for user ${req.user.id}`
    );
  }
});
...
router.post("/addMembersToProject", ensureAuthenticated, async (req, res) => {
  try {
    ...
    console.log(error);
    if (
      error == "Toks vartotojas neegzistuoja!" ||
      error == "Šis vartotojas jau yra pridėtas prie šio projekto"
    ) {
      res.status(500).json({ error });
    } else {
      res
        .status(500)
        .json({ error: "Įvyko nežinoma klaida, prašom pabandyti vėliau!" });
    }

    logger.error(
      `Failed adding new member to project ${req.body.projectId} for user ${req.user.id}`
    );
  }
});
...
async function addTask(userId, taskName) {
  try {
    ...
    const updatedUser = await User.findOneAndUpdate(
      { _id: userId },
      {
        $push: { "calendar.tasks": newTask },
        $inc: { "calendar.totalTasks": 1 },
      },
      { new: true }
    );

    return updatedUser.calendar.tasks;
  } catch (error) {
    logger.error(error);
    console.log();
  }
...
async function changeTaskDueDate(userId, taskId, date) {
  try {
    let result = await User.findOneAndUpdate(
      { _id: userId, "calendar.tasks._id": taskId },
      { $set: { "calendar.tasks.$.dueDate": date } },
      { new: true }
    );
  ...
  try {
    ...
    if (existingProject) {
      for (let i = 0; i < memberList.length; i++) {
        const member = memberList[i].trim();
        ...
        if (!existingUser) {
          ...
          throw "Toks vartotojas neegzistuoja!";
        }

        if (existingProject.members.find((m) => m.name == member)){
          await existingProject.save();
          throw "Šis vartotojas jau yra pridėtas prie šio projekto";
        }

        if (
          existingUser &&
          !existingProject.members.find((m) => m.name == member)
        ) {
          const newMember = {
